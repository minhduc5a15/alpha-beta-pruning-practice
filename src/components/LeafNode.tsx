...
export default function LeafNode({ id, value, state, truth, onUpdate, showResults, isActive, isStepMode, onClick }: Props) {
  ...
  return (
    <div className={`relative flex flex-col items-center gap-1 mt-2 text-slate-300 transition-all ${isActive ? 'scale-110 z-30 transform-gpu' : ''}`}>
       ...
       <div 
         ...
         } ${showResults && !isCorrect ? 'border-rose-500 bg-rose-500/20' : ''} ${showResults && isCorrect ? 'border-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.2)] text-emerald-300' : ''} ${!isStepMode ? 'cursor-pointer hover:border-slate-500' : ''}`}
       >
         {value}
       </div>
